...
function AdvBadge({ adv, t1, t2 }) {
  if (adv === 'team1') return <span style={{ color: '#4f8ef7', fontWeight: 700, fontSize: 12 }}>← {t1}</span>;
  if (adv === 'team2') return <span style={{ color: '#a78bfa', fontWeight: 700, fontSize: 12 }}>{t2} →</span>;
      ...
      {data && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 16, alignItems: 'center' }}>
            <div className="card" style={{ textAlign: 'center', border: '1px solid #4f8ef7' }}>
              <div style={{ fontSize: 18, fontWeight: 800 }}>{t1.name}</div>
              <div style={{ fontSize: 28, fontWeight: 900, color: '#4f8ef7', margin: '8px 0' }}>
            ...
            <div className="card" style={{ textAlign: 'center', border: '1px solid #a78bfa' }}>
              <div style={{ fontSize: 18, fontWeight: 800 }}>{t2.name}</div>
              <div style={{ fontSize: 28, fontWeight: 900, color: '#a78bfa', margin: '8px 0' }}>
                ...
                <div className="card">
                  ...
                  {data.historical_matchups.map((g, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--border)', fontSize: 13 }}>
                      ...
                      <span style={{ fontWeight: 600 }}>{g.team1} <span style={{ color: '#4f8ef7' }}>{g.team1_score}</span></span>
                      <span style={{ color: 'var(--muted)' }}>vs</span>
                      <span style={{ fontWeight: 600 }}><span style={{ color: '#a78bfa' }}>{g.team2_score}</span> {g.team2}</span>
          ...
          {activeTab === 'categories' && (
            <div className="card">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 1fr', gap: 4, fontSize: 12 }}>
                <div style={{ color: '#4f8ef7', fontWeight: 700, padding: '6px 0' }}>{t1.name}</div>
                <div style={{ color: 'var(--muted)', textAlign: 'center', padding: '6px 0' }}>Category</div>
                <div style={{ color: '#a78bfa', fontWeight: 700, padding: '6px 0', textAlign: 'right' }}>{t2.name}</div>
                {data.category_comparison.map((cat, i) => {
                  const oneSided = cat.advantage === 'team1_only' || cat.advantage === 'team2_only';
                  const t1color = cat.advantage === 'team1' ? '#4f8ef7' : cat.advantage === 'even' ? 'var(--muted)' : 'var(--text)';
                  const t2color = cat.advantage === 'team2' ? '#a78bfa' : cat.advantage === 'even' ? 'var(--muted)' : 'var(--text)';
          ...
          {activeTab === 'uc' && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              {[
                { team: t1, rec: data.uc.team1_recommendation, reason: data.uc.team1_reason, color: '#4f8ef7', strength: data.uc.team1_uc_strength },
                { team: t2, rec: data.uc.team2_recommendation, reason: data.uc.team2_reason, color: '#a78bfa', strength: data.uc.team2_uc_strength },
